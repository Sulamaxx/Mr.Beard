...
const PendingOrders: React.FC = () => {
  ...
  const fetchPendingOrders = async () => {
    try {
      ...
      const response = await ApiService.get<ApiResponse>('/v2/user/orders?status=pending');
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="text-white mt-3">Loading pending orders...</p>
      </div>
    );
  }
  ...
  if (orderItems.length === 0) {
    return (
      <div className="text-center py-5">
        <p className="text-white">No pending orders found.</p>
      </div>
    );
  }
          ...
          <tbody>
            {orderItems.map((orderItem, index) => (
              <tr key={`${orderItem.order_id}-${orderItem.item.id}`}>
                ...
                <td>
                  <div className="quantity-container qty-input">
                    <input 
                      type="number" 
